<script>
	...
	function calculate() {
		...
		// Bereken geprojecteerde waarden voor alle parameters
...
<svelte:head>
	<title>{t.titleExpert}</title>
</svelte:head>

<h1>{t.headerExpert}</h1>

<div class="nav-link">
	<a href="/">{t.linkSimple}</a>
</div>
		...
		<div class="parameter-card">
			...
			<div class="input-grid-half">
				<div class="input-group">
					...
					<input 
						type="number" 
						id="replacement-{param.id}" 
						inputmode="decimal" 
						step="0.1"
						bind:value={param.replacement} 
					/>
				</div>
				<div class="input-group">
	...
	<div class="content-box">
		{#if results.percent > 0 && results.driver}
			<div class="result-box">
				{@html t.recommendationText(results.liters, results.percent)}
			</div>
			<p id="summary-driver">
				{@html t.driverText(results.driver.name)}
			</p>
		{:else}
			<div class="result-box">
				{t.errorMsgIneffective}
			</div>
		{/if}
			...
			<tbody>
				{#each results.projections as p (p.id)}
					<tr class:driver-row={results.driver && p.id === results.driver.id}>
						...
						<td>
							<strong>
								{isNaN(p.projectedValue) ? 'N/A' : p.projectedValue.toFixed(2)} {p.unit}
							</strong>
						</td>
					</tr>
				{/each}
