<script>
	...
	import { currentLang } from '$lib/store.js'; // <-- 1. IMPORTEER DE STORE

	let modalVisible = false;
	let readmeContent = 'Loading...';

	// $: t wordt nu automatisch bijgewerkt wanneer de $currentLang store verandert.
	$: t = translations[$currentLang] || translations.en;
	$: isExpertMode = $page.route.id === '/expert';

	function setLanguage(lang) {
		// 2. Schrijf de nieuwe taal naar de store
	...
	onMount(() => {
		...
		// Zet de initiële taal in de store
...
<div class="page-wrapper" class:expert-mode={isExpertMode}>
	...
	<div class="footer-controls">
		...
		<div class="lang-switcher">
			<!-- 3. Gebruik de store-waarde ($currentLang) om de actieve class te bepalen -->
...
<style>
	...
	@media (max-width: 480px) { .page-wrapper { max-width: 100%; padding: 0 10px; } :global(.content-box) { padding: 20px; } :global(.parameter-card) { padding: 12px; } :global(.input-grid-half) { grid-template-columns: 1fr; gap: 10px; } .modal-content { width: 95%; padding: 15px 20px; } }
</style>